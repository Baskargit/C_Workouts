...
int main()
{
    ...
    UpdateFileName(rootNode,"F1.1.0.1.txt","F1.1.0.1-changed.txt");
...
bool UpdateDirName(struct DirNode *node,char oldDirName[],char newDirName[])
{
    ...
    else
    {
        ...
        {
            UpdateDirName(node->firstDir,oldDirName,newDirName);
            return UpdateDirName(node->nextSibling,oldDirName,newDirName);
        }
    }
}
...
bool UpdateFileName(struct DirNode *node,char oldFileName[],char newFileName[])
{
    ...
    else
    {
        ...
        {
            if (UpdateFileName(node->firstDir,oldFileName,newFileName))
            {
                return true;
            } 
            else
            {
                return UpdateFileName(node->nextSibling,oldFileName,newFileName);   
            }
        }   
    }
}
...
bool DeleteDir(struct DirNode *node,char dirName[])
{
    ...
    else
    {
        ...
        {
            DeleteDir(node->firstDir,dirName);
            return DeleteDir(node->nextSibling,dirName);
        }
    }
...
bool DeleteFile(struct DirNode *node,char fileName[])
{
    if (node != NULL && node->firstFile != NULL)
    {
        ...
        else
        {
            ...
            return false; // if no file matched in the current directory
        }
    }
    else
    {
        ...
        {
            DeleteFile(node->firstDir,fileName);
            return DeleteFile(node->nextSibling,fileName);
        }  
    }
